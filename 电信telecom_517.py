...
"""
cron: 37 8 * * *
new Env('电信517任务_全自动互助集合版_品赞代理');

青龙环境变量：
chinaTelecomAccount=手机号#密码&手机号#密码
PROXY_API=品赞代理API地址（可选）
PROXY_TYPE=http 或 socks5（可选，默认http）
...
PROXY_TYPE = os.getenv("PROXY_TYPE", "http")  # http 或 socks5
PROXY_RETRY_TIMES = 3
...
def validate_proxy(proxy_info, timeout=10):
    """验证代理是否可用"""
    if not proxy_info:
        return False
    ...
    try:
        ...
        response = requests.get(PROXY_VALIDATE_URL, proxies=proxies, timeout=timeout)
        if response.status_code == 200:
            ip = response.json().get("origin", "未知")
            printn(f"   ✅ 代理验证通过 | 出口IP: {ip}")
            return True
    except Exception as e:
        printn(f"   ⚠️ 代理验证失败 | 原因: {str(e)}")
    return False
...
def get_valid_proxy(account_name):
    """获取可用代理"""
    if not PROXY_API:
        printn(f"   ℹ️ [{account_name}] 未配置代理 | 使用直连模式")
        return None

    printn(f"   🔌 [{account_name}] 正在获取专属代理...")
    ...
    for i in range(PROXY_RETRY_TIMES):
        try:
            ...
            if not proxy_info:
                printn(f"   ⚠️ [{account_name}] 第{i+1}次获取代理失败 | 响应格式错误")
                continue

            if validate_proxy(proxy_info):
                printn(f"   ✅ [{account_name}] 第{i+1}次代理获取成功")
                return proxy_info
            else:
                printn(f"   ⚠️ [{account_name}] 第{i+1}次代理不可用 | 重试中...")

        except Exception as e:
            printn(f"   ⚠️ [{account_name}] 第{i+1}次获取代理异常 | 原因: {str(e)}")

        if i < PROXY_RETRY_TIMES - 1:
            time.sleep(2)

    printn(f"   ❌ [{account_name}] 代理获取失败 | 切换直连模式")
    return None
...
async def ks(phone, ticket, proxy_info=None):
    ...
    if proxies:
        ...
        printn(f" - 使用代理: {proxy_info['host']}:{proxy_info['port']}")
    else:
        printn(f" - 使用直连模式")
...
async def main():
    ...
    printn(f" - 🔌 代理模式: {'品赞代理' if PROXY_API else '直连'}")
    print("-" * 50)
    ...
    for index, phoneV in enumerate(phone_list):
        ...
        # 获取代理
        proxy_info = get_valid_proxy(masked_phone)
        ...
        if ticket:
            await ks(phone, ticket, proxy_info)
